...
public class frmColors implements Runnable{
	...
	@Override
	public void run() {
		Frame frm = new Frame("Color codes");
		...
		try{
			frm.setLocation(gui.getXas(), gui.getYas());
		}catch (Exception ex){
			frm.setLocation(DSG.GetScreenSizeX(frm)/2 - (ImageObserver.WIDTH/2), 0);
